...
main() {
    ...
    if [[ -z "${GH_APP_ID:-}" ]]; then
        echo "Error: GH_APP_ID environment variable is not set" >&2
        return 1
    fi

    if [[ -z "${GH_APP_INSTALLATION_ID:-}" ]]; then
        echo "Error: GH_APP_INSTALLATION_ID environment variable is not set" >&2
        return 1
    fi

    if [[ -z "${GH_APP_PRIVATE_KEY:-}" ]]; then
        echo "Error: GH_APP_PRIVATE_KEY environment variable is not set" >&2
        return 1
    fi
