...
check_promoted() {
    ...
    if [[ "$promoted" == "null" || -z "$promoted" ]]; then
        ...
        if ! skopeo=$(skopeo $skopeo_mac_args inspect "docker://$promoted" 2>/dev/null); then
            ...
            # Extract build date
            build_date="$(echo "$skopeo" | jq -r '.Labels.["build-date"]' | sed 's/Z$//')"

            # Extract git commit SHA from image labels
            # Try common label names used by Konflux/OpenShift builds
            for label in "io.openshift.build.commit.id" "vcs-ref" "org.opencontainers.image.revision"; do
                if commit_sha=$(echo "$skopeo" | jq -r -e ".Labels[\"$label\"]" 2>/dev/null); then
                    break
                fi
            done

            if [[ "$commit_sha" == "null" ]]; then
                buildtime="REVISION_FAILURE,Failed,"
            else
