...
This directory contains scripts and configurations for centralized golangci-lint v2 version management across multiple repositories. All projects require **Go 1.23+**.
...
3. Download the config file to a temp directory (if no local config exists)
...
2. **Remote config** - downloaded to `/tmp/golangci-lint-config/`
